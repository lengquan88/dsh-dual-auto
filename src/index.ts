// index.ts — 双模型 Auto DSH 插件 (函数插件形态: named exports)
// 消费: fs (持久化) / tools (注册) / llm (真实双模型调用, 可选 ctx.get)
// 发布: @guaikepa/dsh-dual-auto — cordis.yml 引用 name 包名 (免 file:// 路径坑)
// 持久化: 项目 output/dsh_router_{fingerprints,stats}.json + decision_log.jsonl
//   (与 Python dao/model_router.py 格式互通; 周检第 34 项观测)
import type { Context } from '@deepseek-ai/cordis'
import { createCore, fingerprintOf, type RouterCore, type RouterTask } from './core.js'
...
interface LlmLike {
  stream(options: { provider: string; model: string; messages: unknown[] }): AsyncIterable<{
    type?: string
    text?: string
...
export function apply(ctx: Context): void {
  const core: RouterCore = createCore()
  let flushedLog = 0
  let lastPersist = 0
  const stats = core.stats()
  // fs/tools 由 DSH 注入 (Context 类型无此属性, 按注入契约断言)
  const fsApi = (ctx as unknown as {
    fs: {
      resolve(p: string): Promise<unknown>
      readText(t: unknown): Promise<string>
      writeText(t: unknown, c: string, e?: unknown, s?: unknown, p?: unknown): Promise<unknown>
    }
  }).fs

  const readFile = async (rel: string): Promise<string | null> => {
    try { const t = await fsApi.resolve(rel); return await fsApi.readText(t) } catch { return null }
  }
  const writeFile = async (rel: string, content: string): Promise<void> => {
    ...
    } catch (e) { console.error('[dual-auto] write failed', rel, String(e)) }
  }

  const loadState = async (): Promise<void> => {
    const fpMap = new Map<string, { escapes: number; hits: number }>()
    try {
      ...
      if (raw) {
        ...
        for (const k of Object.keys(data)) {
          ...
          } else if (Number(v) > 0) fpMap.set(k, { escapes: Number(v), hits: 0 })
        }
      }
    } catch { /* 坏文件 → 空集 */ }
    ...
    try {
      ...
      if (raw) statMap = JSON.parse(raw) as Record<string, number>
    } catch { /* 缺省 */ }
    ...
    try {
      ...
      if (raw) {
        for (const line of raw.split('\n').filter((l) => l.trim()).slice(-200)) {
          try { logRows.push(JSON.parse(line) as Record<string, unknown>) } catch { /* 坏行 */ }
        }
      }
    } catch { /* 无日志 */ }
    core.restoreState(fpMap, statMap, logRows as never)
  ...
  const schedulePersist = (): void => {
    const now = Date.now()
    if (now - lastPersist < PERSIST_GAP_MS) return
    lastPersist = now
  ...
  async function callModel(task: RouterTask, model: string): Promise<{ ok: boolean; text?: string; ms?: number; error?: string }> {
    ...
    try {
      ...
      const s = core.stats(); s.calls = (s.calls ?? 0) + 1
  ...
  async function runTask(task: RouterTask & { probe?: boolean }): Promise<Record<string, unknown>> {
    ...
    if (wantPro && !res.ok) {
      ...
      if (degraded) { const s = core.stats(); s.degraded = (s.degraded ?? 0) + 1 }
    }
    if (!res.ok) {
      // R181f: 回写调用结果字段 (与 API 面板对账), 触发持久化
      rec.model_used = model; rec.call_ms = undefined; rec.answer_len = undefined
      schedulePersist()
  ...
  const textRender = (_args: unknown, value: unknown): unknown => [{ type: 'text', text: JSON.stringify(value, null, 1) }]
  ...
  function def(tool: { name: string } & Record<string, unknown>): void {
    // 注册即 effect (Cordis 约定), 随插件卸载自动清理
    // tools 由 DSH 注入 (类型上 Context 无此属性, 按注入契约断言)
    const tools = (ctx as unknown as { tools: { register(t: unknown): () => void } }).tools
  ...
  def({
    ...
    execute(args: RouterTask) { return core.route(args) },
  })
  ...
  def({
    ...
    execute(args: RouterTask & { probe?: boolean }) { return runTask(args) },
  })
  ...
  def({
    ...
    execute(args: RouterTask & { correct: boolean }) { return core.markEscape(args, args.correct) },
  })
